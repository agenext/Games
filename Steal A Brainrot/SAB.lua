	...
	function EspBestBrainrot()
	    ...
	    for _, plot in pairs(Plots:GetChildren()) do
	        if plot ~= GetLocalPlot() then
	            ...
	            local MaxPodium, MaxText, MaxRarityColor, MaxMutationColor = nil, "", Color3.fromRGB(255, 255, 255), Color3.fromRGB(255, 255, 255)
	            for _, podium in pairs(AnimalPodiums:GetChildren()) do
	                ...
	                if Mutation then
	                    ...
	                    if MutationLabel then
	                        local Gradient = MutationLabel:FindFirstChildOfClass("UIGradient")
	                        if Gradient and Gradient.Color and Gradient.Color.Keypoints[1] then
	                            MutationColor = Gradient.Color.Keypoints[1].Value
	                        else
	                            MutationColor = MutationLabel.TextColor3
	                        end
	                    end
	                end
	                        ...
	                        if value > MaxGenerationValue then
	                            ...
	                            if Mutation then
	                                MaxText = ("<font color=\"rgb(%d,%d,%d)\">%s</font>\n"):format(
	                                    MutationColor.R*255, MutationColor.G*255, MutationColor.B*255, Mutation
	                                )..NameText.."\n"..("<font color=\"rgb(%d,%d,%d)\">%s</font>"):format(
	                                    RarityColor.R*255, RarityColor.G*255, RarityColor.B*255, Rarity
	            ...
	            for _, podium in pairs(AnimalPodiums:GetChildren()) do
	                local oldBill = podium:FindFirstChild("ESP Best Brainrot")
	                if oldBill then
	                    oldBill:Destroy()
	                end
	            end
