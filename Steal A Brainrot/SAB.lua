	----------------------------------------[[ Others ]]----------------------------------------
	if _G.HasExecuted then 
		repeat task.wait(10) until _G.HasExecuted == false
	end
	...
	function Hop()
		...
		function TPReturner()
			...
			if foundAnything == "" then
				Site = HttpService:JSONDecode(game:HttpGetAsync("https://games.roblox.com/v1/games/" .. PlaceID .. "/servers/Public?sortOrder=Asc&limit=100"))
			else
				Site = HttpService:JSONDecode(game:HttpGetAsync("https://games.roblox.com/v1/games/" .. PlaceID .. "/servers/Public?sortOrder=Asc&limit=100&cursor=" .. foundAnything))
			end
			local ID = ""
	...
	function EspBestBrainrot()
	    ...
	    for _, plot in pairs(Plots:GetChildren()) do
	        if plot ~= GetLocalPlot() then
	            ...
	            if MaxPodium then
	                local Bill = PlotBillboards[plot]
	                local Label
    ...
    if not _G.HasExecuted then
	    ...
	    if queue_on_teleport then
	        queue_on_teleport('loadstring(game:HttpGet("https://raw.githubusercontent.com/agenext/Games/refs/heads/main/Steal%20A%20Brainrot/SAB.lua"))()')
	    end
	end
